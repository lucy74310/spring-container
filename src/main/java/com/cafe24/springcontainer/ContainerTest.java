...
public class ContainerTest {
	...
	public static void testApplicationContext() {
		...
		// name으로 가져오기
		User user = (User) appContext.getBean("usr");
		...
		// 오류 : 같은 타임의 빈이 2개이상 존재하면 타입으로 빈을 가져올 수 없다.
		//appContext.getBean(User.class);
		
		// 오류 : 기본생성자 없으면 오류 . 컨테이너가 기본 생성자로 생성하기 때문이다. 
		...
		//((ConfigurableApplicationContext)appContext).class;
